...
// var Global para não dar conflito
$tc = jQuery.noConflict();



window.onbeforeprint = function() {
    console.log('teste antes de imprimir');
};
...
$tc(function() {
  ...
  // era pra ser apos o onLoad




// fim do $tc(function() { abaixo
});
...
/*	$tc('.valorParcela:visible').each(function() {
		console.log("valor de " + this.className + " eh " + $tc(this).text());
		//$tc('#Extenso_valorParcela:visible').text($tc(this).text());

		if($tc('#'+this.id).length > 0) {
			MostrarExtensoCampoParcela(this.id);
		} else {
			MostrarExtensoCampoParcela("valorParcela");
		}
...
$tc(window).load(function(){
	...
	if(($tc('#valorTotal').is('span') && $tc('#valorTotal').text() == '0,00') || 
		($tc('#valorTotal').is('input') && $tc('#valorTotal').val() == '0,00')) {

		var calcValorTotal = 0;

		$tc('.valorParcela').each(function() {			
			
				var campoAtual = $tc('#'+this.id).is('span') ? $tc('#'+this.id).text() : $tc('#'+this.id).val();
				campoAtual = campoAtual.replace(".", "");
				campoAtual = campoAtual.replace(",", ".");
				calcValorTotal += Number(campoAtual);

		});

		$tc('#valorTotal').is('span') ? $tc('#valorTotal').text(FloatToMoeda(calcValorTotal)) : $tc('#valorTotal').val(FloatToMoeda(calcValorTotal));
		MostrarExtensoCampo("valorTotal");
